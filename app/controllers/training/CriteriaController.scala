...
class CriteriaController extends Controller {
  ...
  def createOrUpdate = Action.async(parse.json) {
    request =>
      ...
      val entity = Json.fromJson[Criteria](input).get
      val response = for {
        results <- CriteriaService.apply.createOrUpdate(entity)
      } yield results
  ...
  def getCriteriaById( id: String) = Action.async {
    request =>
      val response = for {
        results <- CriteriaService.apply.getCriteriaById( id)
      } yield results
