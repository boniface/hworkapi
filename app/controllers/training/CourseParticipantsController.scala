...
class CourseParticipantsController extends Controller {
  ...
  def createOrUpdate = Action.async(parse.json) {
    request =>
      ...
      val entity = Json.fromJson[CourseParticipants](input).get
      val response = for {
        results <- CourseParticipantsService.apply.createOrUpdate(entity)
      } yield results
  ...
  def getCourseParticipants( id: String) = Action.async {
    request =>
      val response = for {
        results <- CourseParticipantsService.apply.getCourseParticipants(id)
      } yield results
